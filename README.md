...
A second frontend lives in `frontend/` (Vite + React + Tailwind v4) and talks to a thin FastAPI server in `server/` that wraps the same simulator pipeline. The Streamlit app stays fully functional — this is an alternative UI, not a replacement.
...
Then open <http://localhost:5173>. The simulator is at `/simulator`, results live at `/results/:configHash`, and `/results/:configHash/diagnostics` shows the correlation panel. The Bayesian MMM tab is currently stubbed as "Coming soon" — use the Streamlit app for MMM in the meantime. See [frontend/README.md](frontend/README.md) for layout, theming, and stack notes.
...
2. **Saturation** (if global and per-channel saturation are on): `linear` (default slope 1 scales impressions), `hill`, or `diminishing_returns` as configured in `saturation_config`. For **`hill`**, `saturation_config.K` is the **half-saturation point in the same units as weekly impressions** (this codebase: impressions = weekly spend ÷ CPM × 1000). If `K` is tiny compared to typical impressions (often \(10^5\)–\(10^6\) at realistic spend), Hill output stays near **1** and behaves like no curve. Rule of thumb: set `K` near **\((\text{spend}_{\min}+\text{spend}_{\max})/2 \div \text{CPM} \times 1000\)** or use the app’s **Suggest K from spend & CPM** when Hill is selected.
